...
class MLP:
    def __init__(self, input_size, output_size, hidden_layer_sizes, hidden_activations, output_activation, loss_function, learning_rate=0.01, n_iter=1000, batch_size=None, tol=0.0001, task='classification', threshold=0.5):
    ...
    def backward_pass(self, activations, Z, y_true):
        ...
        if self.task == 'classification':
            ...
            if self.output_activation != 'linear':
                dZ *= self.apply_activation_derivative(Z[f'Z{L}'], self.output_activation)
    ...
    def compute_loss(self, y_true, y_pred):
        if self.loss_function == 'cross_entropy':
            if y_true.ndim == 1 and self.task != 'binary_classification':  # If y_true is a vector of class labels
            ...
            if self.task == 'classification':
                y = np.array([[0, 1] if label == 1 else [1, 0] for label in y_true])
                return(np.mean(y-y_pred)**2)
            else:
                return np.mean((y_true - y_pred) ** 2)
                
        else:
            raise ValueError(f"Unsupported loss function for regression: {self.loss_function}")

    def predict(self, X):
        activations, _ = self.forward_pass(X)
        output = activations[f'A{len(self.layer_sizes) - 1}']
        if self.task == 'binary_classification':
            return (output >= self.threshold).astype(int)
        if self.task == 'classification':
    ...
    def mse_loss(self, y_true, y_pred):
        if self.task == 'classification':
            ...
            return np.mean(mean_squared_diff)
        else:
            return np.mean((y_true - y_pred) ** 2)
    ...
    def gradient_check(self, X, y, epsilon=1e-7):
        ...
        for key in self.params:
            ...
            if not np.allclose(param_grad, grads[grad_key], rtol=1e-3, atol=1e-3):
